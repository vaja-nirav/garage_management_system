<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Inspection Checklist') }} - {{ $inspectionChecklist->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('inspection-checklists.update', $inspectionChecklist) }}" id="checklistForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $inspectionChecklist->name)" required />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="is_active" name="is_active" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    <option value="1" {{ $inspectionChecklist->is_active ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ !$inspectionChecklist->is_active ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                            
                            <div class="md:col-span-2">
                                <x-input-label for="description" :value="__('Description (Optional)')" />
                                <textarea id="description" name="description" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $inspectionChecklist->description) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="overflow-x-auto mb-4">
                            <table class="min-w-full divide-y divide-gray-200" id="itemsTable">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200" id="itemsBody">
                                    @foreach($inspectionChecklist->items as $index => $item)
                                    <tr class="item-row">
                                        <td class="px-4 py-2">
                                            <input type="text" name="items[{{ $index }}][category]" value="{{ $item->category }}" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm sm:text-sm" placeholder="e.g. Under Hood, Exterior">
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="text" name="items[{{ $index }}][item_name]" value="{{ $item->item_name }}" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm sm:text-sm" placeholder="e.g. Inspect Brake Pads" required>
                                        </td>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                let rowCount = {{ $inspectionChecklist->items->count() }};
