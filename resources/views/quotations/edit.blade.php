<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Quotation') }} - {{ $quotation->quotation_number }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('quotations.update', $quotation) }}" id="quotationForm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <select id="vehicle_id" name="vehicle_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    @foreach($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}" data-customer-id="{{ $vehicle->customer_id }}" {{ $quotation->vehicle_id == $vehicle->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <x-text-input id="quotation_date" name="quotation_date" type="date" class="mt-1 block w-full" value="{{ $quotation->quotation_date->format('Y-m-d') }}" required />
                            </div>

                            <!-- Valid Until -->
                            <div>
                                <x-input-label for="valid_until" :value="__('Valid Until (Optional)')" />
                                <x-text-input id="valid_until" name="valid_until" type="date" class="mt-1 block w-full" value="{{ $quotation->valid_until ? $quotation->valid_until->format('Y-m-d') : '' }}" />
                            </div>
                        ...
                        <div class="overflow-x-auto mb-4">
                            <table class="min-w-full divide-y divide-gray-200" id="itemsTable">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200" id="itemsBody">
                                    ...
                                    <tr class="item-row" {{ $item->product_id ? 'data-product-id='.$item->product_id : '' }}>
                                        <td class="px-4 py-2">
                                            @if($item->product_id)
                                                <input type="hidden" name="items[{{ $index }}][product_id]" value="{{ $item->product_id }}">
                                            @endif
                                            <input type="text" name="items[{{ $index }}][description]" value="{{ $item->description }}" class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm sm:text-sm" placeholder="Part or Custom Service Name" required>
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="items[{{ $index }}][quantity]" value="{{ rtrim(rtrim($item->quantity, '0'), '.') }}" class="qty-input block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm sm:text-sm" min="1" step="0.01" required>
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="items[{{ $index }}][unit_price]" value="{{ $item->unit_price }}" class="price-input block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm sm:text-sm" min="0" step="0.01" required>
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" name="items[{{ $index }}][tax_rate]" value="{{ rtrim(rtrim($item->tax_rate, '0'), '.') }}" class="tax-input block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm sm:text-sm" min="0" step="0.01">
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="text" class="line-total block w-full bg-gray-50 border-gray-300 rounded-md shadow-sm sm:text-sm" value="{{ $item->total }}" readonly>
                                        </td>
                        ...
                        <div class="flex justify-end mb-6">
                            <div class="w-1/3 bg-gray-50 p-4 rounded-md">
                                <div class="flex justify-between mb-2">
                                    ...
                                    <span class="font-medium text-gray-900" id="subtotalDisplay">${{ number_format($quotation->total_amount, 2) }}</span>
                                </div>
                                <div class="flex justify-between mb-2">
                                    <span class="font-medium text-gray-700">Total Tax:</span>
                                    <span class="font-medium text-gray-900" id="taxDisplay">${{ number_format($quotation->tax_amount, 2) }}</span>
                                </div>
                                <div class="flex justify-between border-t border-gray-300 pt-2">
                                    <span class="font-bold text-gray-900 text-lg">Grand Total:</span>
                                    <span class="font-bold text-gray-900 text-lg" id="grandTotalDisplay">${{ number_format($quotation->net_amount, 2) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ $quotation->notes }}</textarea>
                        </div>

                        <div class="flex items-center justify-end">
                            <a href="{{ route('quotations.index') }}" class="mr-4 text-sm text-gray-600 hover:text-gray-900">Cancel</a>
                            <x-primary-button>
                                {{ __('Update Quotation') }}
                            </x-primary-button>
                        </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                let rowCount = {{ count($quotation->items) }};
