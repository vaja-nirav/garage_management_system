<x-app-layout>
            ...
            <form action="{{ route('purchase-returns.store') }}" method="POST" id="returnForm">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl mb-6 border border-slate-100">
                    <div class="p-6 text-gray-900">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div>
                                <label class="block font-black text-xs uppercase tracking-widest text-slate-400 mb-2" for="purchase_id">Select Original Purchase</label>
                                <select name="purchase_id" id="purchase_id" class="mt-1 block w-full border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm select2" required>
                                    <option value="">Search Purchase # or Supplier...</option>
                                    @foreach($purchases as $purchase)
                                        <option value="{{ $purchase->id }}" data-garage="{{ $purchase->garage_id }}">
                                            {{ $purchase->purchase_number }} - {{ $purchase->supplier->company_name }} ({{ \Carbon\Carbon::parse($purchase->purchase_date)->format('d M, Y') }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="return_number" id="return_number" value="RET-{{ strtoupper(uniqid()) }}" class="mt-1 block w-full border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm font-bold" required>
                            </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl mb-6 border border-slate-100">
                    <div class="p-6 text-gray-900">
                            ...
                            <table class="w-full text-sm text-left" id="returnTable">
                                <thead class="text-xs text-slate-500 uppercase bg-slate-50/50 border-b border-slate-100">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 font-black text-center">Purchased Qty</th>
                                        <th class="px-6 py-4 font-black text-center">Return Qty</th>
                                        <th class="px-6 py-4 font-black text-right">Unit Price</th>
                                        <th class="px-6 py-4 font-black text-right">Refund Total</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50">
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-slate-400 font-medium">
                                            Select a purchase above to load items.
                                        </td>
                                    </tr>
                                </tbody>
                                <tfoot class="bg-indigo-600 text-white font-black text-lg border-t-4 border-white">
                                    <tr>
                                        <td colspan="4" class="px-6 py-6 text-right uppercase tracking-widest">Total Refund Amount</td>
                        ...
                        <div class="mt-8">
                            <label class="block font-black text-xs uppercase tracking-widest text-slate-400 mb-2" for="notes">Return Reason / Notes</label>
                            <textarea name="notes" id="notes" class="mt-1 block w-full border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-2xl shadow-sm" rows="3" placeholder="Enter reason for return..."></textarea>
                        </div>

                        <div class="mt-10 flex items-center justify-end border-t border-slate-50 pt-8">
                            <a href="{{ route('purchase-returns.index') }}" class="text-slate-400 hover:text-slate-600 px-6 py-3 font-bold text-xs uppercase tracking-widest transition-colors">Discard</a>
                            <button type="submit" id="submitBtn" class="ml-4 inline-flex items-center px-12 py-4 bg-indigo-600 border border-transparent rounded-xl font-black text-xs text-white uppercase tracking-[0.2em] hover:bg-indigo-700 shadow-xl shadow-indigo-200 transform transition hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed" disabled>
    ...
    <script>
        ...
        function loadPurchaseItems(purchaseId) {
            ...
            $('#returnTable tbody').html('<tr><td colspan="5" class="px-6 py-12 text-center text-slate-400 font-medium animate-pulse uppercase tracking-widest">Fetching purchase items...</td></tr>');
            
            $.get(`{{ url('purchase-returns/get-items') }}/${purchaseId}`, function(items) {
                $('#loadingItems').addClass('hidden');
                let rows = '';
                
                if (items.length === 0) {
                    rows = '<tr><td colspan="5" class="px-6 py-12 text-center text-rose-400 font-bold uppercase tracking-widest">No items found for this purchase.</td></tr>';
                                ...
                                <td class="px-6 py-5 text-center">
                                    <div class="flex items-center justify-center">
                                        <input type="number" name="items[${item.product_id}][quantity]" value="0" min="0" max="${item.quantity}" 
                                               class="return-qty-input w-24 border-slate-200 rounded-xl text-center font-black focus:ring-indigo-500 focus:border-indigo-500 text-indigo-600" 
